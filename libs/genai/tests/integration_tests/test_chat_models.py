...
_AGENTIC_VIDEO_MODEL = "gemini-3.7-flash"
...
@pytest.mark.flaky(retries=3, delay=1)
def test_agentic_video_understanding_youtube(backend_config: dict) -> None:
    """Agentic processing of a YouTube URL passed as a standard `video` block."""
    if backend_config.get("vertexai"):
        pytest.skip("Agentic video understanding is Gemini API only")

    llm = ChatGoogleGenerativeAI(model=_AGENTIC_VIDEO_MODEL, **backend_config)
    message = HumanMessage(
        content=[
            {
                "type": "video",
    ...
    assert isinstance(result, AIMessage)
    # Exercises the server-side tool-use token accounting: agentic processing
    # reports `tool_use_prompt_token_count`, so the parts only sum once it is
    # folded into `input_tokens`.
    _check_usage_metadata(result)
    assert isinstance(result.text, str)
    assert result.text.strip()

    # The steps prove the model navigated the video rather than sampling frames.
    assert isinstance(result.content, list)
    step_types = [
        block.get("type")
        for block in result.content
        if isinstance(block, dict)
        and block.get("type") in ("server_tool_call", "server_tool_result")
    ]
    assert "server_tool_call" in step_types
    assert "server_tool_result" in step_types

...
_IMAGE_OUTPUT_MODEL = "models/gemini-2.0-flash-preview-image-generation"
...
def test_chat_google_genai_invoke_no_image_generation_without_modalities() -> None:
    ...
    modalities."""
    llm = ChatGoogleGenerativeAI(model=_IMAGE_OUTPUT_MODEL)
    # NOTE: This API changed with `google-genai` and now raises an error
    # instead of returning a response with no image.

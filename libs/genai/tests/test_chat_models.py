...
@pytest.mark.parametrize(
    "raw_candidate, expected, legacy_function_call",
    [
        (
        ...
        (
            {
                "content": {
                    "parts": [{"function_call": glm.FunctionCall(name="Ping", args={})}]
                }
            },
            AIMessage(
                content="",
                additional_kwargs={},  # expect EMPTY
                tool_calls=[
                    create_tool_call(
                        name="Ping", args={}, id="00000000-0000-0000-0000-00000000000"
                    )
                ],
            ),
            False,
...
def test_parse_response_candidate(
    raw_candidate: Dict, expected: AIMessage, legacy_function_call: bool
) -> None:
    with patch("langchain_google_genai.chat_models.uuid.uuid4") as uuid4:
        ...
        result = _parse_response_candidate(
            response_candidate, legacy_function_call=legacy_function_call
        )

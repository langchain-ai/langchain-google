...
class _BaseVertexAIVectorStore(VectorStore):
    ...
    def delete(self, ids: Optional[List[str]] = None, **kwargs: Any) -> Optional[bool]:
        ...
        Args:
            ...
            **kwargs (Any): If added metadata={}, deletes the documents that match the metadata filter and the parameter
            ids is not needed.
        Returns:
        ...
        if (not ids and not metadata) or (ids and metadata):
            raise ValueError("You should provide ids (as list of id's) or a metadata filter for deleting documents.")
        if metadata:

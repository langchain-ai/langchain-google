"""Vertex AI Claude prompt caching middleware."""
...
try:
    ...
    msg = (
        "VertexPromptCachingMiddleware requires 'langchain' to be installed. "
...
class VertexPromptCachingMiddleware(AgentMiddleware):
    ...
    def __init__(
        ...
        Args:
            ...
            min_messages_to_cache: The minimum number of messages until the
                cache is used.
            unsupported_model_behavior: The behavior to take when an unsupported
                model is used.
        """
        self.type = type
        self.ttl = ttl
    ...
    def _should_apply_caching(self, request: ModelRequest) -> bool:
        ...
        Raises:
            ValueError: If the model is unsupported and behavior is `'raise'`.
        """
        if not isinstance(request.model, ChatAnthropicVertex):
            msg = (
                "VertexPromptCachingMiddleware caching middleware only supports "
                "ChatAnthropicVertex models, not instances of "
                f"{type(request.model)}"
            )
    ...
    async def awrap_model_call(
        ...
        """Modify the model request to add cache control blocks asynchronously.
...
def _tag_system_message(
    ...
    """Tag the last content block of a system message with cache control."""
...
def _tag_tools(
    ...
    """Tag the last tool with cache control via its extras dict."""
    if not tools:
        return tools

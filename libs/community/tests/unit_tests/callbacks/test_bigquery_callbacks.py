...
async def test_async_log_parsing_error(
  ...
  with (
    ...
    assert (
        f"Failed to parse content: {error_message}"
        in logged_row["error_message"]
    )
...
def test_trace_id_registry_child_run_propagation() -> None:
  ...
  child_trace_id = registry.register_run(
...
async def test_async_trace_id_registry_missing_parent_behavior() -> None:
  ...
  trace_id = await registry.register_run(
      run_id, parent_run_id=unknown_parent_id
  )
...
def test_langgraph_node_detection(
  ...
  assert (
      handler._is_langgraph_root_invocation(serialized, uuid4(), metadata)
      is False
  )
  ...
  assert (
      handler._is_langgraph_root_invocation(serialized, None, metadata) is True
  )
